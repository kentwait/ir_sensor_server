...
from api.config import DEVICES_SHELVE_PATH
from ircodec.command import CommandSet
from api.common.util import success, error
from irdevice.control import Control
import shelve
...
class Device(Resource):
    ...
    def list_devices(self):
        with shelve.open(DEVICES_SHELVE_PATH) as shl:
    ...
    def list_command_ids(self, device_id):
        with shelve.open(DEVICES_SHELVE_PATH) as shl:
            ...
            command_ids = shl[device_id].commands
    ...
    def post(self, device_id=None):
        ...
        with shelve.open(DEVICES_SHELVE_PATH) as shl:
            if device_id not in shl.keys():
                return error('device_id not found', code=400)
            if args['command_id'] not in shl[device_id].commands:
                return error('command_id not found', code=400)
            device = shl[device_id]

        # TODO: Retrieves state data from local sqlite

        # Sends signal using ir blaster
        try:
            control_id, command_id = args['command_id'].split('.')
            device.__getattribute__(control_id).__getattribute__(command_id)()
